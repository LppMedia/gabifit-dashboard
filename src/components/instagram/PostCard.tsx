...
export function PostCard({ post, onDelete, onStatusChange, onEdit }: PostCardProps) {
  ...
  return (
    <Card className="group relative flex flex-col gap-0 overflow-hidden border-border/60 bg-card p-0 transition-all duration-200 hover:border-border hover:shadow-lg hover:shadow-black/20">
      {/* ── Visual banner ─────────────────────────────────────────────── */}
      <div
        className={cn(
      ...
      <div className="flex flex-col gap-3 p-4">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex flex-wrap gap-1.5">
            <Badge
              variant="outline"
              className={cn("text-[11px]", POST_TYPE_COLORS[post.type])}
            >
              {POST_TYPE_LABELS[post.type]}
            </Badge>
            <Badge
              variant="outline"
              className={cn("text-[11px]", STATUS_COLORS[post.status])}
            >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-transparent opacity-0 transition-opacity hover:bg-muted group-hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
            >
        ...
        {/* Hashtags */}
        {post.hashtags && (
          <p className="flex items-center gap-1 truncate text-[11px] text-pink-400/80">
